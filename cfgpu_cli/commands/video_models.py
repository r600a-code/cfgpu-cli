...
def run(provider: Optional[str] = None, page: int = 1):
    ...
    Args:
        provider: 可选的模型提供商筛选 (如: 字节跳动, Kling, 万相, HappyHorse等)
        page: 页码，默认第1页
    """
    ...
    print(f"\n{'=' * 70}")
    if provider:
        print(f"  视频模型 - {provider} (第 {page}/{total_pages} 页)")
    else:
        print(f"  视频模型 - 视频生成模型列表 (第 {page}/{total_pages} 页)")
    print(f"{'=' * 70}")
    ...
    print(f"  可用提供商: 字节跳动, Kling, 万相, HappyHorse, 千问")
    print(f"{'=' * 70}\n")
